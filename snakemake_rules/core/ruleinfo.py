...
regex_wildcard_constraints = re.compile(
    r"(?P<key>[a-zA-Z0-9_]+)[ ]*=[ ]*[\"\'](?P<constraint>[^=]+)[ ]*[\"\']")
# The tacit assumption is that @workflow() always ends with a newline
regex_workflow = re.compile(
    r"@workflow\.(?P<block>[a-zA-z0-9]+)\((?P<content>[\s\S]*?)\)\n")
...
def parse_rule(rule, prefix=None):
    ...
    d['wildcard_constraints'] = {
        k: v for k, v in regex_wildcard_constraints.findall(
