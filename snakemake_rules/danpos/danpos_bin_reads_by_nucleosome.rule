...
rule danpos_bin_reads_by_nucleosome:
    ...
    output: bed=["{{prefix}}.{nucleosome}.bed".format(nucleosome=x) for x in range(len(config['danpos']['bins']))]
    ...
    run:
        ...
        bins = np.array([x for item in config['danpos']['bins'] for x in item])
        d = {'count': {k:0 for k in ['NA'] + list(range(len(config['danpos']['bins'])))},
             'rule': 'danpos.danpos_bin_reads_by_nucleosome', 
             'date': utc_time()}
        for s in samfile:
