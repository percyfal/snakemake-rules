...
rule danpos_dpos:
    ...
    """
    params: cmd=config['danpos']['cmd'],
            options=config['danpos']['dpos']['options']
    input: bg="{prefix}.0.bed", nuc=["{{prefix}}.{nucleosome}.bed".format(nucleosome=x) for x in range(1, len(config['danpos']['bins']))]
    output: log=os.path.join("{prefix}", "danpos", "dpos.log")
    threads: config['danpos']['dpos']['threads']
    ...
    run:
        ...
        clist = [" {cmd} dpos {path} {options} -o {outdir}".format(**d)]
