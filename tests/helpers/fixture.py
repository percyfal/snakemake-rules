...
fixture_ext = {
    ...
    "_1.fastq.gz": "data/tiny/CHS_1.fastq.gz",
    "_2.fastq.gz": "data/tiny/CHS_2.fastq.gz",
...
local_ext = {
    'config': normpath(pjoin(
        dirname(__file__), '../config/config.yaml')),
    'sampleinfo': normpath(pjoin(
        dirname(__file__), '../config/sampleinfo.csv')),
}

fixture_ext.update(local_ext)

regex = re.compile(r"(?P<ext>({}))['\"]?$".format(
    "|".join("{}".format(x) for x in sorted(fixture_ext.keys()))))
...
def set_inputmap(rule):
    ...
    if not INPUTCONFIG.get(rule['app'], {}).get(
            rule['name'], {}).get('ft', None) is None:
        inputmap = inputmap + \
                   [(x, fixture_ext[x])
                    for x in INPUTCONFIG[rule['app']][rule['name']]['ft']]
...
def set_output(rule, wildcards):
    ...
    if not INPUTCONFIG.get(
            rule['app'], {}).get(rule['name'], {}).get('output', None) is None:
